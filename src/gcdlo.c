...
#if defined(GCDLO_FORCED_RES_WIDTH) != defined(GCDLO_FORCED_RES_HEIGHT)
#error "GCDLO_FORCED_RES_WIDTH and GCDLO_FORCED_RES_HEIGHT must be defined together"
#endif

#if defined(GCDLO_FORCED_RES_HZ) && !defined(GCDLO_FORCED_RES_WIDTH)
#error "GCDLO_FORCED_RES_HZ requires GCDLO_FORCED_RES_WIDTH and GCDLO_FORCED_RES_HEIGHT"
#endif

#if defined(GCDLO_FORCED_RES_WIDTH)
#if GCDLO_FORCED_RES_WIDTH <= 0 || GCDLO_FORCED_RES_HEIGHT <= 0
#error "GCDLO_FORCED_RES_WIDTH and GCDLO_FORCED_RES_HEIGHT must be positive"
#endif
#ifdef GCDLO_FORCED_RES_HZ
#if GCDLO_FORCED_RES_HZ <= 0
#error "GCDLO_FORCED_RES_HZ must be positive"
#endif
#endif
#define GCDLO_FORCED_RES_ENABLED 1
#endif
...
{
#ifdef GCDLO_FORCED_RES_ENABLED
    dlo_mode_t desc = {0};
    dlo_retcode_t ret;
#endif
    dlo_mode_t *pMode;
...
#ifdef GCDLO_FORCED_RES_ENABLED
    desc.view.base = 0;
    desc.view.width = GCDLO_FORCED_RES_WIDTH;
    desc.view.height = GCDLO_FORCED_RES_HEIGHT;
    desc.view.bpp = 24;
#ifdef GCDLO_FORCED_RES_HZ
    desc.refresh = GCDLO_FORCED_RES_HZ;
#else
    desc.refresh = pMode->refresh ? pMode->refresh : 60;
#endif

    ret = dlo_set_mode(uid, &desc);

    if (!DLO_CheckRetcode("dlo_set_mode", ret)) {
        printf("gc: DisplayLink forced mode %ux%u not accepted, ignoring.\n",
               desc.view.width,
               desc.view.height);
    }
    ...
    if (!pMode) {
        printf("gc: dlo_get_mode failed after forced mode set\n");
        return GCFALSE;
    }
#endif
